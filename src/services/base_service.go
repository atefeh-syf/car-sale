...
func (s *BaseService[T, Tc, Tu, Tr]) Update(ctx context.Context, id int, req *Tu) (*Tr, error) {
	...
	model := new(T)
	tx := s.Database.WithContext(ctx).Begin()
	if err := tx.Model(model).Where("id = ? And deleted_by is null", id).Updates(*updateMap).Error; err != nil {
		tx.Rollback()
...
func (s *BaseService[T, Tc, Tu, Tr]) Delete(ctx context.Context, id int) error {
	...
	if cnt := tx.Model(model).Where("id = ? And deleted_by is null", id).Updates(deleteMap).RowsAffected; cnt == 0 {
